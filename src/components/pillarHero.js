import React from 'react';
import {Link, graphql, useStaticQuery} from 'gatsby';
import Img from 'gatsby-image';
import BackgroundImage from 'gatsby-background-image';
import {QuoteContainer, QuoteBox} from './component-styles/quoteStyles';
import ScheduleDemo from './ctas/scheduleDemo';
import fluidImage from '../helpers/imageHelper';

const pillarHero = props => {
    const data = useStaticQuery(graphql`
        query {
            bkg: file(relativePath: {eq: "blobs.png"}) {
                ...fluidImage
            }
            keusler: file(relativePath: {eq: "Keusler.png"}) {
                ...fluidImage
            }
        }
    `);
    return (
        <BackgroundImage
            fluid={data.bkg.sharp.fluid}
            style={{
                width: '100%',
                backgroundSize: '100%',
                backgroundRepeat: 'no-repeat',
                overflowX: 'hidden'
            }}
        >
            <div className="row justify-content-center my-5">
                <div className="col-12 col-md-6">
                    <h1 className="text-center">{props.heading}</h1>
                    <h2 className="text-center lead">{props.subheading}</h2>
                    <div className="mt-5 text-center">
                        <ScheduleDemo />
                    </div>
                </div>
            </div>
            <div className="container">
                <QuoteContainer className="mx-auto position-relative pt-4">
                    <Img fluid={data.keusler.sharp.fluid} alt="" />
                    <QuoteBox className="bg-white p-3 text-center shadow">
                        <h2 className="font-italic quote">
                            The sky's the limit with the PCIS GOLD EHR. It is
                            going to be customized to you.
                        </h2>
                        <p className="quote-name">
                            Dr. Melissa Keusler, OD - Keusler Eyecare
                        </p>
                        <Link to="/" className="text-primary">
                            Watch Video
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                version="1.1"
                                id="Capa_1"
                                x="0px"
                                y="0px"
                                viewBox="0 0 60 60"
                                style={{
                                    enableBackground: 'new 0 0 60 60',
                                    marginBottom: '-3.5px'
                                }}
                                width="17px"
                                height="17px"
                            >
                                <g>
                                    <path
                                        d="M45.563,29.174l-22-15c-0.307-0.208-0.703-0.231-1.031-0.058C22.205,14.289,22,14.629,22,15v30   c0,0.371,0.205,0.711,0.533,0.884C22.679,45.962,22.84,46,23,46c0.197,0,0.394-0.059,0.563-0.174l22-15   C45.836,30.64,46,30.331,46,30S45.836,29.36,45.563,29.174z M24,43.107V16.893L43.225,30L24,43.107z"
                                        fill="#03a9f4"
                                    ></path>
                                    <path
                                        d="M30,0C13.458,0,0,13.458,0,30s13.458,30,30,30s30-13.458,30-30S46.542,0,30,0z M30,58C14.561,58,2,45.439,2,30   S14.561,2,30,2s28,12.561,28,28S45.439,58,30,58z"
                                        fill="#03a9f4"
                                    ></path>
                                </g>
                            </svg>
                        </Link>
                    </QuoteBox>
                </QuoteContainer>
            </div>
        </BackgroundImage>
    );
};

export default pillarHero;
